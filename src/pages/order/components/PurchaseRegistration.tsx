...
export const PurchaseRegistration: React.FC<PurchaseRegistrationProps> = ({
  ...
  const handleSaveSale = () => {
    ...
    if (isNew) {
      addOrder(newPurchase);
      setIsNew(false);
    } else {
      updateOrder(newPurchase);
    }
  ...
  // 발주 → 매입등록 버튼 클릭 시
  const handleOrderToPurchase = () => {
    if (!form) return;
    addPurchase({
      ...form,
      id: String(Date.now()), // 고유 id 부여
    });
    alert("매입으로 복사되었습니다!");
    navigate("/purchase");
  };
    ...
    <div className="flex flex-col gap-2 bg-white rounded-[4px] w-full">
      ...
      <div className="flex justify-between items-center p-2 pt-2 px-4">
        <div className="flex items-center">
          <h2 className="text-xl font-bold">발주서 등록</h2>
        </div>
      ...
      <div className="flex justify-between gap-4 border-t border-[#E5E7EB] py-4 px-4">
        ...
        <div className="flex gap-4">
          <GreenButton onClick={handleOrderToPurchase}>매입등록</GreenButton>
        </div>
      </div>
      <OfficialPriceDialog
